...
/**
 * Initializes the profile page by populating the avatar, username,
 * using the active user's data from session storage. The avatar displays the first letter of the username in uppercase.
 * The username label is updated to reflect the current user's information.
 */
const initProfile = () => {
...
/**
 * Handles the profile saving process when the "Save" button is clicked. It retrieves the new username creates an updated user object based on the active user's data,
 * and updates the localStorage with the new information. If the username is changed,it also updates the online status in the ChatStore. Finally, it updates the session
...
/**
 * Handles the logout process when the "Logout" button is clicked. It updates the online status of the active user to false in the ChatStore and then calls the SessionManager's
 *  logout method to clear the session and redirect the user to the sign-in page.This ensures that the user's online status is accurately reflected in the chat application
 *  and that they are properly logged out of their session.
 */
